...
for epoch in range(train_epoch):
    ...
    for batch_idx, (hazy, clean) in enumerate(train_loader):
        ...
        writer.add_scalars(
            "training_img",
            {"img loss_l1": smooth_loss_l1.item(), "perceptual": perceptual_loss.item(), "msssim": msssim_loss_.item()},
            iteration,
        )
        writer.add_scalars(
            "GAN_training", {"d_loss": D_loss.item(), "d_score": real_out.item(), "g_score": fake_out.item()}, iteration
        )
