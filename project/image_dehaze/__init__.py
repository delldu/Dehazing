...
def image_client(name, input_files, output_dir):
    ...
    for filename in image_filenames:
        ...
        context = cmd.dehaze(filename, output_file)
...
def image_server(name, host="localhost", port=6379):
    ...
    def do_service(input_file, output_file, targ):
        print(f"  dehaze {input_file} ...")
        try:
            input_tensor = todos.data.load_tensor(input_file)
            output_tensor = model_forward(model, device, input_tensor, DEHAZE_ZEROPAD_TIMES)
    ...
    return redos.image.service(name, "image_dehaze", do_service, host, port)
...
def image_predict(input_files, output_dir):
    ...
    for filename in image_filenames:
        ...
        predict_tensor = model_forward(model, device, input_tensor, DEHAZE_ZEROPAD_TIMES)
        output_file = f"{output_dir}/{os.path.basename(filename)}"
...
def video_service(input_file, output_file, targ):
    ...
    print(f"  dehaze {input_file}, save to {output_file} ...")
    progress_bar = tqdm(total=video.n_frames)

    def dehaze_video_frame(no, data):
    ...
    video.forward(callback=dehaze_video_frame)
...
def video_client(name, input_file, output_file):
    ...
    context = cmd.dehaze(input_file, output_file)
    redo = redos.Redos(name)
    redo.set_queue_task(context)
...
def video_server(name, host="localhost", port=6379):
    return redos.video.service(name, "video_dehaze", video_service, host, port)
